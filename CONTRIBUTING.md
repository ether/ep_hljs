# Contributing to ep_hljs

Thanks for helping improve `ep_hljs` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_hljs` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_hljs from your local clone
pnpm run plugins i --path ../ep_hljs

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_hljs/` are picked up after a server restart.
...
* Etherpad version, ep_hljs version, browser + OS
* Steps to reproduce
